...
import com.ii.domain.base.event.ChangeStatusEvent;
import com.ii.domain.base.service.ContinuationService;

/**
 * 全局共享的 handler 资源对象，该holder持有的handler{@link Handler}集合都是等待用户的
 * 发起的event{@link ChangeStatusEvent}。
 * 1，如果该handler已处理event{@link ChangeStatusEvent},则从队里中移除该handler；
 * 2，所有设备请求过来的handler都会按策略加入到该holder集合中{@link ContinuationService}；
 * 3，当收到用户发起的event{@link ChangeStatusEvent}，
